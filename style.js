...
<script type="module">
  ...
  // --- Login Page Constants & Functions ---
  const firebaseConfig = {
    apiKey: "AIzaSyBDoNMXyJbN5ziB5Zt5rEsb6Ltj0zgGTgA",
    authDomain: "lzr-aura.firebaseapp.com",
    databaseURL: "https://lzr-aura-default-rtdb.firebaseio.com",
    projectId: "lzr-aura",
    storageBucket: "lzr-aura.firebasestorage.app",
    messagingSenderId: "825644714875",
    appId: "1:825644714875:android:f4843d7aa13b603ed94bde"
  };
  ...
  function getDeviceId() {
    ...
    document.getElementById("deviceId").innerText = `Device ID: ${id}`;
    return id;
  }
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (data.deviceId && data.deviceId !== deviceId) {
        errorDiv.innerText = "This key is already in use on another device.";
        return;
      }
      ...
      localStorage.setItem("access_key_expiry", data.expiry || "none");
      
      userWinCount = data.winCount || 0;
      userLossCount = data.lossCount || 0;
  ...
  function handleLogout() {
      ...
      localStorage.removeItem("access_key_expiry");
  ...
  function getKey() {
    const deviceId = getDeviceId();
    const message = encodeURIComponent(`Hello t.me/TECH_WITH_ANIKESH, I need an Access Key for Device ID: ${deviceId}`);
    window.open(`https://t.me/TECH_WITH_ANIKESH001?text=${message}`, '_blank');
  }
  ...
  let last100Results = [];
  let lastHistoryHash = '';
  let fetchInterval, timeInterval, keyTimerInterval;
  ...
  function startKeyTimer(expiryTimestamp) {
      clearInterval(keyTimerInterval);
      const timerElement = document.getElementById("keyTimer");

      if (expiryTimestamp === "none" || isNaN(expiryTimestamp)) {
          timerElement.innerHTML = `Key Expiry: Lifetime`;
          return;
      }
      ...
      keyTimerInterval = setInterval(() => {
          ...
          if (remaining <= 0) {
              timerElement.innerHTML = `Key Expiry: Expired!`;
              clearInterval(keyTimerInterval);
              return;
          }
